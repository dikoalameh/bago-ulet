    ...
    <div id="filterModal" onclick="outsideClick(event)"
        ...
        <div class="relative flex items-center justify-center bg-white w-[400px] p-6 rounded-[10px] shadow-md">
            <form action="" class="w-full px-2">
                ...
                <button type="button" onclick="updateTable(); closeSettingsModal('filterModal')" class="mt-4 bg-primary text-white tracking-widest uppercase px-4 py-2 rounded">
                    Apply
                </button>
            </form>
        </div>
                    ...
                    <tr data-date="{{ $review->created_at->format('Y-m-d') }}">
                        ...
                        <td>
                            @if($review->reviewer_ID)
                                <a href="{{ route('iacuc.view-review-files', ['protocolId' => $review->protocol_ID, 'reviewerId' => $review->reviewer_ID]) }}">
